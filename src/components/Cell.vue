  ...
  <td class="td_hover" @click="create">
    ...
    <span v-for="(i,index) in info[1]" :key="index">
      <span v-if="(i.day_of_week == info[2])&&(i.lesson==info[3])">
        ...
        <p
          v-else-if="info[4]=='group'"
        >{{i.teacher.last_name}} {{i.teacher.first_name[0]}}.{{i.teacher.middle_name[0]}}.</p>
  ...
  computed: {
    g_t_type() {
      for (var i in this.info[1]) {
        if (
          this.info[1][i].day_of_week == this.info[2] &&
          this.info[1][i].lesson == this.info[3]
        )
          if (this.info[4] == "teacher") return this.info[1][i].group.id;
